...
export class GraphitiMemoryClient {
  ...
  async writeDecision(input: MemoryWriteInput): Promise<WriteReceipt> {
    return this.ingest("decision", input);
  }
  async writeOutcome(input: MemoryWriteInput): Promise<WriteReceipt> {
    return this.ingest("observation", input);
  }
  async writeSemanticFact(input: MemoryWriteInput): Promise<WriteReceipt> {
  ...
  private async callToolOnce<T>(name: string, args: Record<string, unknown>): Promise<T> {
    ...
    try {
      ...
      throw new MemoryRpcError(
        `memory tool ${name} returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async post(
    ...
    try {
      ...
      throw new MemoryRpcError(
        `memory HTTP ${response.status} returned invalid MCP content: ${error instanceof Error ? error.message : String(error)}`,
        undefined,
        undefined,
        response.status,
      );
    ...
    if (!response.ok)
      throw new MemoryRpcError(
        payload.error?.message ?? `memory HTTP ${response.status}`,
        payload.error?.code,
        payload.error?.data,
        response.status,
      );
