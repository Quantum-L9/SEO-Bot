...
vi.mock('bullmq', () => {
  class Queue {
    constructor(public name: string, _opts?: unknown) {}
    async add(name: string, data: any, opts: any) { bull.queueAdds.push({ queueName: this.name, name, data, opts }); }
    async close() { bull.closed.queues++; }
  }
  class Worker {
    ...
    on() { /* record nothing — events not under test */ }
    async close() { bull.closed.workers++; }
  }
  return { Queue, Worker, Job: class {} };
});
vi.mock('ioredis', () => ({
  Redis: class { constructor(_url?: unknown, _opts?: unknown) {} async quit() { bull.closed.redisQuit++; } },
}));
...
vi.mock('../../src/core/database/index.js', () => ({
  getDb: () => ({
    ...
    update: () => ({ set: (payload: any) => { dbState.updateSets.push(payload); return { where: () => Promise.resolve([]) }; } }),
  }),
  schema: { jobExecutions: { id: 'je.id' }, clients: { active: 'clients.active' } },
}));

import { fanoutChildJobId, Scheduler, getScheduler, jobDefinitions } from '../../src/core/scheduler.js';
...
describe('Scheduler.processJob — client fan-out (GAP-006)', () => {
  it('enqueues one deterministic-id child per active client, threading clientConfig', async () => {
    ...
    expect(a.data).toMatchObject({ clientId: 'client-a', clientDomain: 'a.com', clientConfig: { tenant: 'A' } });
  });
  ...
  it('marks the execution failed and rethrows when the handler rejects', async () => {
    const scheduler = new Scheduler();
    await scheduler.start();
    scheduler.registerHandler('vitals:check-all-sources', vi.fn().mockRejectedValue(new Error('handler boom')));
