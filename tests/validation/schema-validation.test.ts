...
function evidence(): GateEvidence {
  return {
    ...
    required: true,
    status: "PASS",
    repository: { name: "Quantum-L9/SEO-Bot", commit_sha: sha, branch: "main", dirty: false },
    execution: {
      command: ["npm", "test"],
      cwd: ".",
    ...
    assertions: [
      { id: "test.exit", description: "passes", expected: 0, actual: 0, result: "PASS" },
    ],
    findings: [],
    blocked_by: [],
    unknowns: [],
...
describe("evidence schema validation", () => {
  it("accepts a complete PASS gate", () =>
    expect(() => validateGateEvidence(evidence())).not.toThrow());
      ...
      required_gates: [
        {
          ...
          required: true,
          status: "PASS",
          evidence_path: "container.json",
          evidence_digest: digest,
        },
      ],
      blocking_findings: [],
      unknowns: [],
      generated_at: new Date(1).toISOString(),
