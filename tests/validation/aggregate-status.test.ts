...
} from "../../scripts/validation/types.js";

const strict: ProfileDefinition = {
  gates: ["test"],
  allow_pass_with_findings: false,
  blocked_is_failure: true,
};
const tolerant: ProfileDefinition = {
  gates: ["test"],
  allow_pass_with_findings: true,
  blocked_is_failure: true,
};
const externallyTolerant: ProfileDefinition = {
  gates: ["test"],
  allow_pass_with_findings: true,
...
describe("aggregateStatus", () => {
  ...
  it("promotes non-blocking findings", () =>
    expect(aggregateStatus(["PASS", "PASS_WITH_FINDINGS"])).toBe("PASS_WITH_FINDINGS"));
  it("preserves BLOCKED over findings", () =>
    expect(aggregateStatus(["PASS_WITH_FINDINGS", "BLOCKED"])).toBe("BLOCKED"));
  it("preserves UNKNOWN over BLOCKED", () =>
    expect(aggregateStatus(["BLOCKED", "UNKNOWN"])).toBe("UNKNOWN"));
  it("preserves FAIL over every other state", () =>
    expect(aggregateStatus(["PASS", "UNKNOWN", "FAIL"])).toBe("FAIL"));
