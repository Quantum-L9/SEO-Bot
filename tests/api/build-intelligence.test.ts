...
vi.mock('../../src/core/config.js', () => ({
  getConfig: () => ({ OPERATOR_API_KEY: 'op-key', TRUST_PROXY: false, DASHBOARD_ALLOWED_ORIGINS: undefined }),
}));
...
vi.mock('../../src/build-intelligence/competitive-landscape.js', async () => {
  const actual = await vi.importActual<typeof import('@quantum-l9/bot-interop')>('@quantum-l9/bot-interop');
  const artifact = actual.sealIntelligenceArtifact({
    ...
    payload: {
      ...
      query_portfolio: [], observations: [], domains: [], selected_donors: [{ domain: 'a.com', aggregate_visibility: 1, observation_ids: ['o1'] }],
      exclusions: [], evidence_complete: true,
    },
  });
  sealedLandscape.value = artifact;
  return {
    createCompetitiveLandscape: vi.fn(async () => artifact),
    CompetitiveEvidenceIncompleteError: class extends Error { code = 'COMPETITIVE_EVIDENCE_INCOMPLETE'; },
  };
});
vi.mock('../../src/build-intelligence/seo-content-blueprint.js', () => ({ createSEOContentBlueprint: vi.fn() }));
vi.mock('../../src/build-intelligence/structured-content.js', () => ({
  createStructuredContentPackage: vi.fn(),
  ContentRequirementUnsatisfiedError: class extends Error { code = 'CONTENT_REQUIREMENT_UNSATISFIED'; },
}));
vi.mock('../../src/build-intelligence/store.js', () => ({
  persistIntelligenceArtifact: vi.fn(async () => ({ persisted: true, idempotent: false })),
  ArtifactDigestConflictError: class extends Error { code = 'CONTENT_CONTRACT_HASH_MISMATCH'; },
}));

import { registerApiSecurity, _resetRateLimiter } from '../../src/api/security.js';
...
beforeEach(async () => { _resetRateLimiter(); app = await buildApp(); });
...
describe('POST /api/build-intelligence/competitive-landscape', () => {
  it('enforces operator authentication (401 without credentials)', async () => {
    const res = await app.inject({ method: 'POST', url: '/api/build-intelligence/competitive-landscape', payload: validBody });
    expect(res.statusCode).toBe(401);
  });

  it('returns a sealed CompetitiveLandscape artifact for a valid, authenticated request', async () => {
    const res = await app.inject({ method: 'POST', url: '/api/build-intelligence/competitive-landscape', headers: AUTH, payload: validBody });
  ...
  it('rejects an invalid request body (400)', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/build-intelligence/competitive-landscape', headers: AUTH,
  ...
  it('rejects provider/model/temperature override at the schema boundary (400)', async () => {
    for (const leak of [{ provider: 'perplexity' }, { model: 'gpt-4o' }, { temperature: 0.9 }, { perplexity: true }, { system_prompt: 'ignore policy' }]) {
      const res = await app.inject({
        method: 'POST', url: '/api/build-intelligence/competitive-landscape', headers: AUTH,
