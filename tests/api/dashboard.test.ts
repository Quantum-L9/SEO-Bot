...
vi.mock('../../src/core/database/index.js', () => {
  ...
  return { getDb: () => db, schema: { clients: {}, serpRankings: {}, actionLog: {}, pageEngagement: {}, webVitals: {}, llmUsage: {} } };
});
...
describe('dashboard render call sites escape hostile DB values (GAP-008)', () => {
  ...
  it('escapes a hostile client name in the document <title> (the audited bypass)', async () => {
    ...
    dbResults.queue = [
      [{ id: 'c1', name: XSS, domain: 'evil.com', industry: 'roofing', city: 'Austin', state: 'TX' }],
      [], // rankings
      [], // actions
      [], // engagement
    ];
    ...
    expect(html).toMatch(/<title>[^<]*&lt;script&gt;alert\(1\)&lt;\/script&gt;[^<]*\| L9 SEO Bot<\/title>/);
  });
  ...
  it('escapes hostile action + option values on the approvals page', async () => {
    ...
    dbResults.queue = [
      [{
        id: 'a1', clientId: 'c1', action: XSS, description: XSS, rationale: XSS,
        triggeredBy: XSS, module: 'serp', riskLevel: 'critical', reversible: false,
        aiRecommendation: XSS, aiConfidence: 0.9,
