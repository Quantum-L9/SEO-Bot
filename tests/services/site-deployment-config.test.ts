...
describe('siteConfigFromStoredClient', () => {
  ...
  it('fails closed when the repository contract is incomplete', () => {
    const config = siteConfigFromStoredClient({ site_deployment: { ...canonical.site_deployment, verifiedCommitSha: undefined } }, { TENANT_GITHUB_TOKEN: 'gh-token' });
    expect(config.dryRun).toBe(true);
  });


  it('fails closed when verification evidence is structurally incomplete', () => {
    const config = siteConfigFromStoredClient({ site_deployment: { ...canonical.site_deployment, contractDigest: undefined } }, { TENANT_GITHUB_TOKEN: 'gh-token' });
    expect(config.dryRun).toBe(true);
  });
