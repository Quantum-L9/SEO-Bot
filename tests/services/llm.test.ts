...
vi.mock('@quantum-l9/llm-router', () => {
  ...
  const TaskType = {
    CLASSIFICATION: 'classification', EXTRACTION: 'extraction', SCORING: 'scoring',
    CONTENT_GENERATION: 'content_generation', STRATEGIC_REASONING: 'strategic_reasoning',
    COMPETITOR_RESEARCH: 'competitor_research', CITATION_CHECK: 'citation_check',
    LAYOUT_VALIDATION: 'layout_validation',
  } as const;
  const TaskComplexity = { TRIVIAL: 'trivial', LOW: 'low', MEDIUM: 'medium', HIGH: 'high' } as const;
  class L9LLMRouter {
...
vi.mock('../../src/core/database/index.js', () => ({
  getDb: () => ({
    ...
    select: () => ({ from: () => ({ where: () => Promise.resolve([{ totalCost: dailyTotal.value }]) }) }),
  }),
...
import { BudgetExhaustedError, TaskType, TaskComplexity } from '@quantum-l9/llm-router';

const okResponse = { content: 'ok', inputTokens: 10, outputTokens: 20, cost: 0.01, model: 'test-model' };
...
describe('LlmService.execute — gate ordering + memory + usage (GAP-005)', () => {
  it('requires a clientId before any provider or memory work', async () => {
    ...
    await expect(
      svc.execute({ type: TaskType.CLASSIFICATION, complexity: TaskComplexity.LOW } as any, 'sys', 'user'),
  ...
  it('appends the hydrated memory context to the system prompt and forwards options', async () => {
    ...
    await svc.execute(
      { clientId: 'c1', type: TaskType.CONTENT_GENERATION, complexity: TaskComplexity.MEDIUM, description: '[web-vitals] fix' } as any,
      'SYSTEM',
      'USER',
      { images },
    );
  ...
  it('records usage on a successful call (module parsed from the description tag)', async () => {
    const svc = new LlmService();
    await svc.execute(
      { clientId: 'c1', type: TaskType.EXTRACTION, complexity: TaskComplexity.LOW, description: '[serp-intelligence] parse' } as any,
      'sys', 'user',
    );
...
describe('LlmService public methods route the correct task type + complexity (GAP-005)', () => {
  it('classify → CLASSIFICATION / LOW', async () => {
    ...
    expect(executeMock.mock.calls[0][0]).toMatchObject({ type: TaskType.CLASSIFICATION, complexity: TaskComplexity.LOW });
  });

  it('score → SCORING / LOW', async () => {
    await new LlmService().score('prompt', 'c1', 'web-vitals' as any, 'purpose');
    expect(executeMock.mock.calls[0][0]).toMatchObject({ type: TaskType.SCORING, complexity: TaskComplexity.LOW });
  });
