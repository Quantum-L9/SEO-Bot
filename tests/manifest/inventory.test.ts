...
async function repository(): Promise<string> {
  ...
  await writeFile(
    path.join(root, "manifest", "ownership.yaml"),
    JSON.stringify({
...
describe("repository inventory", () => {
  ...
  it("produces deterministic sorted entries", async () => {
    ...
    expect(first.entries.map((item) => item.path)).toEqual(
      [...first.entries.map((item) => item.path)].sort((left, right) => left.localeCompare(right)),
    );
  });
});
