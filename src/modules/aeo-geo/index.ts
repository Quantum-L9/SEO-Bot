...
Generate 3-5 questions.`,
  ...
  logger.info(
    { clientDomain, citedCount, totalChecked, citationRate: `${citationRate}%` },
...
async function optimizeFaqs(job: Job): Promise<void> {
  ...
  const uncitedQueries = recentCitations.filter((c) => !c.cited).map((c) => c.query);

  const citedQueries = recentCitations.filter((c) => c.cited).map((c) => c.query);

  logger.info(
    { clientDomain, uncitedCount: uncitedQueries.length, citedCount: citedQueries.length },
    "Optimizing FAQs",
  );
