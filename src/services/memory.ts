...
function getMemoryClient(): GraphitiMemoryClient | null {
  ...
  client = new GraphitiMemoryClient({
    baseUrl: config.L9_MEMORY_URL,
    bearerToken: config.L9_MEMORY_TOKEN,
  });
  return client;
}
...
async function canonicalClientId(databaseClientId: string): Promise<string> {
  ...
  try {
    const [clientRow] = await getDb()
      .select({ config: schema.clients.config })
      .from(schema.clients)
      .where(
        or(
...
async function guarded<T>(
  operation: string,
  fn: (memory: GraphitiMemoryClient) => Promise<T>,
): Promise<T | undefined> {
  ...
  try {
    ...
    logger.warn(
      ...
      "Governed memory operation failed",
    );
    return undefined;
  }
}
...
export async function hydrateSeoContext(
  ...
  const result = await guarded("hydrate", (memory) =>
    memory.hydrate({
...
export async function promoteMeasuredOutcomes(): Promise<{
  recorded: number;
  promoted: number;
  failed: number;
}> {
    ...
    try {
      ...
      if (!recordId) {
        ...
        if (!receipt.record_id)
          throw new Error(`Memory write ${receipt.status} returned no record_id`);
      ...
      await db
        ...
        .where(eq(schema.actionOutcomes.id, outcome.id));
      logger.warn(
        { outcomeId: outcome.id, error: message },
        "Measured outcome memory recording failed",
      );
      failed += 1;
    }
  }
  ...
  for (const group of groups.values()) {
    ...
    try {
      ...
      logger.warn(
        { outcomeIds: group.map((item) => item.outcome.id), error: message },
        "Corroborated memory promotion failed",
      );
      failed += group.length;
    }
  }
