...
function failed(
  probes: MaintenanceProbe[],
  code: MaintenanceReadinessError["code"],
  message: string,
): never {
...
export async function verifyMaintenanceReadiness(
  ...
  try {
    githubToken =
      resolveSecretRef(contract.site.maintenance.github_credential_ref, env)?.value ?? "";
    probes.push({
      name: "credential",
      ok: true,
      detail: contract.site.maintenance.github_credential_ref,
    });
  } catch (error) {
    probes.push({
      name: "credential",
  ...
  if (contract.site.maintenance.vercel_deploy_hook_ref) {
    try {
      ...
      probes.push({
        name: "deploy_hook",
        ok: true,
        detail: contract.site.maintenance.vercel_deploy_hook_ref,
      });
    } catch (error) {
      probes.push({
        name: "deploy_hook",
  ...
  try {
    ...
    if (repoResponse.status === 404)
      failed(probes, "REPOSITORY_NOT_FOUND", `${repo.full_name} not found or token lacks access`);
    if (!repoResponse.ok)
      failed(probes, "GITHUB_PROBE_FAILED", `Repository probe returned ${repoResponse.status}`);
    ...
    if (repo.repository_id && String(repoBody.id ?? "") !== repo.repository_id) {
      failed(
        probes,
        "REPOSITORY_ID_MISMATCH",
        `expected id ${repo.repository_id}, observed ${String(repoBody.id ?? "")}`,
      );
    }
    probes.push({
      name: "repository",
      ok: true,
      detail: String(repoBody.full_name ?? repo.full_name),
    });

    const refResponse = await githubGet(
      fetchImpl,
      `${GITHUB_API}/repos/${repo.full_name}/git/ref/heads/${branchRefPath(repo.branch)}`,
      githubToken,
    );
    if (refResponse.status === 404)
      failed(probes, "BRANCH_NOT_FOUND", `branch ${repo.branch} not found`);
    if (!refResponse.ok)
      failed(probes, "GITHUB_PROBE_FAILED", `Branch probe returned ${refResponse.status}`);
    ...
    if (observedHead !== repo.commit_sha)
      failed(
        probes,
        "COMMIT_MISMATCH",
        `expected ${repo.commit_sha}, observed ${observedHead || "UNKNOWN"}`,
      );
    ...
    for (const path of contract.site.maintenance.required_paths) {
      ...
      if (!response.ok)
        failed(probes, "REQUIRED_PATH_MISSING", `${path} missing at ${repo.commit_sha}`);
  ...
  return {
    ...
    ...(contract.site.maintenance.vercel_deploy_hook_ref
      ? { vercelDeployHookRef: contract.site.maintenance.vercel_deploy_hook_ref }
      : {}),
    probes,
  };
}
