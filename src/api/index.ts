...
export async function startApiServer(port: number = 3100): Promise<void> {
  ...
  app.get('/health', async () => {
    ...
    return { status: dbOk ? 'healthy' : 'degraded', timestamp: new Date().toISOString(), uptime: process.uptime(), components: { database: dbOk ? 'connected' : 'disconnected', scheduler: scheduler.isRunning() ? 'active' : 'stopped' }, version: '1.0.0' };
  });

  app.get('/api/status', async () => {
    const db = getDb();
    const clients = await db.select().from(schema.clients).where(eq(schema.clients.active, true)).orderBy(schema.clients.name);
    return { status: 'running', activeClients: clients.length, clients: clients.map(c => ({ id: c.id, domain: c.domain, industry: c.industry })), uptime: process.uptime() };
  });

  app.get('/api/llm-spend', async () => {
    const llm = getLlmService();
    return { dailySpend: await llm.getDailySpend(), timestamp: new Date().toISOString() };
  });
  ...
  app.get<{ Params: { clientId: string } }>('/api/clients/:clientId/report', async (request) => {
    ...
    const citationRate = citations.length > 0 ? (citations.filter(c => c.cited).length / citations.length * 100).toFixed(1) : 'N/A';
    return { period: { from: oneWeekAgo.toISOString(), to: new Date().toISOString() }, rankings: { total: rankings.length, improved: improved.length, declined: declined.length, topMovers: improved.slice(0, 5).map(r => ({ keyword: r.keyword, from: r.previousPosition, to: r.position })) }, vitals: { latestLcp: vitals[0]?.lcp || null, latestCls: vitals[0]?.cls || null, latestInp: vitals[0]?.inp || null }, linkBuilding: { newProspects: newProspects.length, readyForOutreach: newProspects.filter(p => p.status === 'ready').length, outreachSent: newProspects.filter(p => p.status === 'outreach_queued').length }, aeo: { queriesChecked: citations.length, citationRate: `${citationRate}%` } };
  });
  ...
  app.post<{ Params: { clientId: string }; Body: { module: string } }>('/api/clients/:clientId/trigger', async (request) => {
    ...
    const scheduler = getScheduler();
    const validModules = ['serp:check-rankings','serp:competitor-analysis','serp:generate-surpass-plan','vitals:check-all-sources','aeo:check-citations','aeo:optimize-faqs','links:discover-prospects','links:process-outreach','behavior:pull-engagement','behavior:generate-insights'];
  ...
  app.get('/api/token-budget', async () => {
    ...
    return { date: today, month: today.slice(0, 7), todayActions: outcomes.length, message: 'Detailed token tracking available in logs (structured JSON)' };
  });
