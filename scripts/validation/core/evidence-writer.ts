...
function isTextMediaType(mediaType: string): boolean {
  return (
    mediaType.startsWith("text/") || /(?:json|yaml|xml|javascript|typescript)/i.test(mediaType)
  );
}
...
export class EvidenceWriter {
  ...
  async initialize(): Promise<void> {
    try {
      await stat(this.runDir);
      throw new Error(`Evidence run already exists: ${this.runDir}`);
    } catch (error) {
  ...
  async writeLog(
    ...
    if (!/^[a-z0-9][a-z0-9-]*$/.test(gateId))
      throw new Error(`Invalid gate id for log path: ${gateId}`);
  ...
  async recordArtifact(sourcePath: string, mediaType: string): Promise<ArtifactRecord> {
    ...
    const requested = path.isAbsolute(sourcePath)
      ? sourcePath
      : path.join(this.repositoryRoot, sourcePath);
    ...
    if (isTextMediaType(mediaType))
      content = Buffer.from(redactText(content.toString("utf8")), "utf8");
  ...
  async writeGate(
    evidenceWithoutDigest: Omit<GateEvidence, "evidence_digest">,
...
function renderSummary(report: RunReport): string {
  ...
  const rows = report.gates
    .map((gate) => `| \`${gate.gate_id}\` | ${gate.required ? "yes" : "no"} | **${gate.status}** |`)
    .join("\n");
