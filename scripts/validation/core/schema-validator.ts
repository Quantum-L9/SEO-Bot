...
export function validateGateEvidence(value: GateEvidence): void {
  ...
  assert(
    value.artifacts.every((item) => isSha256(item.sha256) && item.size_bytes >= 0),
    "artifact records are invalid",
  );
  ...
  if (value.status === "PASS_WITH_FINDINGS") {
    ...
    assert(
      value.findings.every((item) => !item.blocking),
      "PASS_WITH_FINDINGS cannot contain blocking findings",
    );
...
export function validateRunReport(value: RunReport): void {
  ...
  assert(/^[a-f0-9]{40}$/.test(value.repository.commit_sha), "commit_sha must be a full SHA-1");
  assert(
    isDateTime(value.started_at) && isDateTime(value.completed_at),
    "run timestamps must be date-time",
  );
  ...
  assert(
    new Set(value.gate_order).size === value.gate_order.length,
    "gate_order must not contain duplicates",
  );
  assert(
    value.gates.every((gate, index) => gate.gate_id === value.gate_order[index]),
    "gate_order must match gate records",
  );
  assert(
    value.gates.every((gate) => isSha256(gate.evidence_digest) && gate.evidence_path.length > 0),
    "gate evidence references are invalid",
  );
  assert(
    value.blocking_findings.every((item) => item.blocking),
    "blocking_findings must contain only blocking findings",
  );
  const expected = aggregateStatus(
    value.gates.filter((gate) => gate.required).map((gate) => gate.status),
  );
...
export function validateReleaseReceipt(value: ReleaseReceipt): void {
  ...
  assert(
    value.profile === "release" || value.profile === "production",
    "release receipt profile is invalid",
  );
  assert(
    value.repository.name === "Quantum-L9/SEO-Bot",
    "release receipt repository name is invalid",
  );
  assert(
    /^[a-f0-9]{40}$/.test(value.repository.commit_sha),
    "release receipt commit_sha must be a full SHA-1",
  );
  ...
  assert(
    value.required_gates.every((gate) => gate.required),
    "release receipt must contain only required gates",
  );
  assert(
    value.blocking_findings.every((item) => item.blocking),
    "release receipt findings must be blocking",
  );
  if (value.overall_status === "PASS" || value.overall_status === "PASS_WITH_FINDINGS") {
    assert(
      typeof value.image.digest === "string" && /^sha256:[a-f0-9]{64}$/.test(value.image.digest),
