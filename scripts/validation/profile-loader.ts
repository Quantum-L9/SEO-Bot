...
function validateProfile(
  ...
  value: unknown,
): asserts value is ProfileDefinition {
  if (!value || typeof value !== "object")
    throw new Error(`Invalid validation policy: profile ${name} is required`);
  const definition = value as Partial<ProfileDefinition>;
  if (
    !Array.isArray(definition.gates) ||
    definition.gates.length === 0 ||
    definition.gates.some((gate) => typeof gate !== "string" || gate.length === 0)
  ) {
  ...
  if (
    typeof definition.allow_pass_with_findings !== "boolean" ||
    typeof definition.blocked_is_failure !== "boolean"
  ) {
    throw new TypeError(
      `Invalid validation policy: profile ${name} must define boolean result policies`,
    );
  }
}
...
export async function loadValidationPolicy(repositoryRoot: string): Promise<ValidationPolicy> {
  ...
  try {
    ...
    throw new Error(
      `validation/policy.yaml must use JSON-compatible YAML syntax: ${(error as Error).message}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Invalid validation policy: schema_version, policy_version, and profiles are required",
    );
  }
