...
function deriveStatus(
  ...
  if (assertions.some((item) => item.result === "FAIL") || findings.some((item) => item.blocking))
    return "FAIL";
...
async function preflightGate(context: GateContext): Promise<GateResult> {
  ...
  const presentLockfiles = (
    await Promise.all(
      lockfiles.map(async (name) => [name, await exists(path.join(context.root, name))] as const),
    )
  )
  ...
  const requiredFiles = [
    ...
    "validation/schemas/gate-evidence.schema.json",
    "validation/schemas/run-report.schema.json",
  ...
  const missingFiles = (
    await Promise.all(
      requiredFiles.map(
        async (name) => [name, await exists(path.join(context.root, name))] as const,
      ),
    )
  )
    .filter(([, present]) => !present)
  ...
  const assertions: ValidationAssertion[] = [
    assertion(
      ...
      "v22.x",
      process.version,
      process.version.startsWith("v22.") ? "PASS" : "FAIL",
    ),
    assertion(
    ...
    assertion(
      "preflight.lockfile",
      "Exactly one npm lockfile exists",
      ["package-lock.json"],
      presentLockfiles,
      presentLockfiles.length === 1 && presentLockfiles[0] === "package-lock.json"
        ? "PASS"
        : "FAIL",
    ),
    assertion(
      "preflight.required-files",
      "Required assurance files exist",
      [],
      missingFiles,
  ...
  const schemaKeys = [...configText.matchAll(/^\s{2}([A-Z][A-Z0-9_]+):\s*z\./gm)].map(
    (match) => match[1],
  );
  ...
  const unexplainedExampleKeys = envKeys.filter(
    (key) => !schemaKeys.includes(key) && !infrastructureOnly.has(key),
  );
  assertions.push(
    assertion(
...
function parseScriptFileReferences(command: string): string[] {
  ...
  for (const match of command.matchAll(/\b((?:src|scripts|dist)\/[^\s"']+\.(?:ts|js|mjs|cjs))\b/g))
      ...
      if (forbidden.test(line))
        findings.push(
          finding(
            ...
            "major",
            `Forbidden unfinished-work marker in ${relative}:${index + 1}`,
            relative,
            true,
            [`${relative}:${index + 1}`],
          ),
...
export async function sourceGate(context: GateContext): Promise<GateResult> {
  ...
  const files = (
    await Promise.all(scanRoots.map((root) => walk(path.join(context.root, root))))
  ).flat();
  ...
  const forbidden = new RegExp(
    String.raw`\b(?:${marker}|${unimplemented})\b|${placeholderBehavior}`,
    "i",
  );
  ...
  const legacy = [
    ...
    "validation/db_checks.jsonl",
    "validation/validation_report.yaml",
  ];
  const staticEvidence = (
    await Promise.all(
      legacy.map(
        async (relative) => [relative, await exists(path.join(context.root, relative))] as const,
      ),
    )
  )
    .filter(([, present]) => present)
  ...
  const assertions = [
    assertion(
      "source.forbidden-markers",
      "Production and tooling code has no unfinished-work markers",
      [],
      findings.map((item) => item.evidence_refs[0]),
      findings.length === 0 ? "PASS" : "FAIL",
    ),
    assertion(
      "source.script-targets",
      "Every package script source target exists",
      [],
      missingTargets,
      missingTargets.length === 0 ? "PASS" : "FAIL",
    ),
    assertion(
      "source.package-manager-refs",
      "Operational files use npm only",
      [],
      nonNpmRefs,
    ...
    assertion(
      ...
      "Legacy pass-only evidence is absent",
      [],
      staticEvidence,
...
async function commandGate(
  ...
  return {
    ...
    assertions: [
      assertion(
        `${gateId}.exit`,
        `${[command, ...args].join(" ")} exits successfully`,
    ...
    findings: passed
      ? []
      : [finding(`${gateId}.command-failed`, "critical", `${gateId} command failed`, gateId)],
  };
}
async function buildGate(context: GateContext): Promise<GateResult> {
  ...
  result.assertions = [
    ...
    assertion(
      ...
      true,
      outputPresent,
      outputPresent ? "PASS" : "FAIL",
    ),
    assertion(
      "build.output-fresh",
      "dist/index.js was produced by this run",
      true,
      outputFresh,
      outputFresh ? "PASS" : "FAIL",
    ),
  ];
...
function collectDocumentationViolations(
  ...
  const historical =
    relative === "docs/alignment_report.md" && text.startsWith("# SUPERSEDED HISTORICAL REPORT");
  ...
  if (!historical)
    for (const pattern of patterns)
      if (pattern.test(text)) prohibitedClaims.push(`${relative}: ${pattern.source}`);
...
export async function claimsGate(context: GateContext): Promise<GateResult> {
  ...
  const discoveredDocumentation = (
    await Promise.all(
      ["docs", "adr"].map(async (directory) =>
        (
  ...
  const assertions = [
    ...
    assertion(
      "claims.readiness",
      "Current docs contain no unsupported readiness claim",
      [],
      prohibitedClaims,
...
async function waitForPostgresReady(
  ...
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const check = await runCommand(
      "docker",
      ["exec", containerName, "pg_isready", "-U", VALIDATION_DB_USER, "-d", databaseName],
      {
        cwd: context.root,
        timeoutMs: 10_000,
      },
...
async function databaseGate(context: GateContext): Promise<GateResult> {
  if (!(await dockerAvailable(context.root))) {
    return {
      ...
      assertions: [
        assertion("database.docker", "Docker daemon is available", true, false, "UNKNOWN"),
      ],
    };
  }
  ...
  try {
    const start = await runCommand(
      ...
      [
        ...
        "-P",
        "-e",
        `POSTGRES_USER=${VALIDATION_DB_USER}`,
        "-e",
        `POSTGRES_PASSWORD=${VALIDATION_DB_PASSWORD}`,
        "-e",
    ...
    stdout += start.stdout;
    stderr += start.stderr;
    if (start.execution.exit_code !== 0)
      throw new Error("Unable to start disposable PostgreSQL container");
    const ready = await waitForPostgresReady(context, containerName, databaseName);
    assertions.push(
      assertion(
        "database.ready",
        "Disposable PostgreSQL becomes ready",
        true,
        ready,
        ready ? "PASS" : "FAIL",
      ),
    ...
    const migrationEnv = syntheticRuntimeEnv(
      validationDatabaseUrl("127.0.0.1", port, databaseName),
    );
    const first = await runCommand("npm", ["run", "migrate"], {
      cwd: context.root,
      env: migrationEnv,
      timeoutMs: 5 * 60 * 1000,
    });
    ...
    assertions.push(
      assertion(
        ...
        0,
        first.execution.exit_code,
        first.execution.exit_code === 0 ? "PASS" : "FAIL",
      ),
    );
    const second = await runCommand("npm", ["run", "migrate"], {
      cwd: context.root,
      env: migrationEnv,
      timeoutMs: 5 * 60 * 1000,
    });
    ...
    assertions.push(
      assertion(
        ...
        0,
        second.execution.exit_code,
        second.execution.exit_code === 0 ? "PASS" : "FAIL",
      ),
    );
    const tableQuery = await runCommand(
      "docker",
      [
        "exec",
        containerName,
    ...
    stdout += tableQuery.stdout;
    stderr += tableQuery.stderr;
    const actualTables = tableQuery.stdout
      .split(/\r?\n/)
      .map((item) => item.trim())
      .filter(Boolean);
    const schemaSources = [
    ...
    assertions.push(
      assertion(
        "database.tables",
        "Every Drizzle table exists after migration",
        [],
        missingTables,
        missingTables.length === 0 ? "PASS" : "FAIL",
      ),
    );
    const journalQuery = await runCommand(
      "docker",
      [
        "exec",
        containerName,
    ...
    stdout += journalQuery.stdout;
    stderr += journalQuery.stderr;
    const journalCount = Number(journalQuery.stdout.trim());
    const journalValid =
      journalQuery.execution.exit_code === 0 && Number.isInteger(journalCount) && journalCount > 0;
    assertions.push(
      assertion(
        "database.migration-journal",
        "Drizzle migration journal records applied migrations",
        ">0",
        journalQuery.execution.exit_code === 0 ? journalCount : journalQuery.stderr.trim(),
        journalValid ? "PASS" : "FAIL",
      ),
    );
    const receiptPath = path.join(
      context.root,
      ".validation-tmp",
      `${suffix}-database-receipt.json`,
    );
    ...
    return {
      status: deriveStatus(assertions, findings),
      assertions,
      findings,
      stdout,
      stderr,
    ...
    if (cleanup.execution.exit_code !== 0 && !/No such container/i.test(cleanup.stderr)) {
      findings.push(
        finding("database.cleanup", "major", "Disposable database cleanup failed", "database"),
      );
...
async function waitForContainerDependencies(
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    const pgReady = await runCommand(
      "docker",
      ["exec", postgresName, "pg_isready", "-U", VALIDATION_DB_USER, "-d", databaseName],
      {
        cwd: context.root,
        timeoutMs: 10_000,
      },
...
async function waitForContainerHealth(
  context: GateContext,
  appName: string,
): Promise<{ healthy: boolean; healthBody: string }> {
  for (let attempt = 0; attempt < 60; attempt += 1) {
    const health = await runCommand(
      "docker",
      ["exec", appName, "curl", "--fail", "--silent", "http://127.0.0.1:3100/health"],
      {
        cwd: context.root,
        timeoutMs: 10_000,
      },
    );
    if (health.execution.exit_code === 0)
      return { healthy: true, healthBody: health.stdout.trim() };
    await new Promise((resolve) => setTimeout(resolve, 500));
  }
...
function recordContainerHealthStatus(healthBody: string, assertions: ValidationAssertion[]): void {
  try {
    ...
    assertions.push(
      assertion("container.health-json", "Health response is JSON", true, healthBody, "FAIL"),
    );
  }
}
...
async function cleanupContainerResources(
  ...
  for (const resource of [appName, redisName, postgresName]) {
    const cleanup = await runCommand("docker", ["rm", "--force", resource], {
      cwd: context.root,
      timeoutMs: 30_000,
    });
    if (cleanup.execution.exit_code !== 0 && !/No such container/i.test(cleanup.stderr)) {
  ...
  const networkCleanup = await runCommand("docker", ["network", "rm", networkName], {
    cwd: context.root,
    timeoutMs: 30_000,
  });
  if (networkCleanup.execution.exit_code !== 0 && !/not found/i.test(networkCleanup.stderr)) {
    findings.push(
      finding(
        "container.cleanup.network",
        "minor",
        "Unable to remove validation network",
        "runtime-platform",
        false,
      ),
    );
  ...
  const imageCleanup = await runCommand("docker", ["image", "rm", "--force", tag], {
    cwd: context.root,
    timeoutMs: 60_000,
  });
  if (imageCleanup.execution.exit_code !== 0 && !/No such image/i.test(imageCleanup.stderr)) {
    findings.push(
      finding(
        "container.cleanup.image",
        "minor",
        "Unable to remove validation image",
        "runtime-platform",
        false,
      ),
    );
...
async function containerGate(context: GateContext): Promise<GateResult> {
  if (!(await dockerAvailable(context.root))) {
    return {
      ...
      blocked_by: [{ type: "environment", name: "docker", reason: "Docker daemon is unavailable" }],
      assertions: [
        assertion("container.docker", "Docker daemon is available", true, false, "UNKNOWN"),
      ],
    };
  }
      ...
      blocked_by: [
        {
          ...
          reason: "Private packages require read:packages authentication",
        },
      ],
      assertions: [
        assertion(
          "container.package-auth",
          "Private package token is present",
          true,
          false,
          "UNKNOWN",
        ),
  ...
  const append = (result: Awaited<ReturnType<typeof runCommand>>): void => {
    stdout += result.stdout;
    stderr += result.stderr;
  };
  try {
    const build = await runCommand(
      ...
      [
        "build",
        "--secret",
        "id=npm_token,env=NODE_AUTH_TOKEN",
        "-f",
        "docker/Dockerfile",
        "-t",
        tag,
    ...
    append(build);
    primaryExecution = build.execution;
    assertions.push(
      assertion(
        "container.build",
        "Production image builds",
        0,
        build.execution.exit_code,
    ...
    const inspect = await runCommand(
      "docker",
      ["image", "inspect", tag, "--format", "{{.Config.User}}"],
      {
        cwd: context.root,
        timeoutMs: 30_000,
      },
    ...
    const imageIdResult = await runCommand(
      "docker",
    ...
    assertions.push(
      assertion(
        ...
        "Production image has a content digest",
    ...
    assertions.push(
      assertion(
        "container.production-dependencies",
        "Development dependencies are absent",
        0,
        deps.execution.exit_code,
        deps.execution.exit_code === 0 ? "PASS" : "FAIL",
      ),
    );
    const network = await runCommand("docker", ["network", "create", networkName], {
      cwd: context.root,
      timeoutMs: 30_000,
    });
    append(network);
    if (network.execution.exit_code !== 0) throw new Error("Unable to create validation network");
    const postgres = await runCommand(
      "docker",
      [
        "run",
        "--detach",
    ...
    append(postgres);
    if (postgres.execution.exit_code !== 0)
      throw new Error("Unable to start validation PostgreSQL");
    const redis = await runCommand(
      "docker",
      [
        "run",
        "--detach",
    ...
    const ready = await waitForContainerDependencies(
      context,
      postgresName,
      redisName,
      databaseName,
    ...
    assertions.push(
      assertion(
        "container.dependencies-ready",
        "Disposable dependencies become ready",
        true,
        ready,
        ready ? "PASS" : "FAIL",
      ),
    ...
    const envArgs = Object.entries(runtimeEnv).flatMap(([name, value]) =>
      typeof value !== "string" ? [] : ["-e", `${name}=${value}`],
    );
    ...
    assertions.push(
      assertion(
        ...
        "Production image applies migrations",
        0,
        migration.execution.exit_code,
        migration.execution.exit_code === 0 ? "PASS" : "FAIL",
      ),
    );
    ...
    assertions.push(
      assertion(
        "container.health",
        "Production container reaches health endpoint",
        true,
        healthy,
        healthy ? "PASS" : "FAIL",
      ),
    ...
    findings.push(
      finding("container.execution", "critical", (error as Error).message, "runtime-platform"),
    );
    const logs = await runCommand("docker", ["logs", appName], {
      cwd: context.root,
      timeoutMs: 30_000,
    });
    append(logs);
  } finally {
    await cleanupContainerResources(
      context,
      { appName, redisName, postgresName, networkName, tag },
      findings,
      append,
    );
  }
...
async function runtimeHealthGate(_context: GateContext): Promise<GateResult> {
  ...
  if (!baseUrl)
    return {
      ...
      blocked_by: [
        {
          type: "environment",
          name: "SEO_BOT_BASE_URL",
          reason: "No deployed SEO Bot URL was provided",
        },
      ],
      assertions: [
        assertion("runtime-health.url", "A deployed URL is provided", true, false, "UNKNOWN"),
      ],
    };
  ...
  try {
    ...
    return {
      status: "FAIL",
      assertions: [
        assertion("runtime-health.request", "Health endpoint is reachable", true, false, "FAIL"),
      ],
      findings: [
        finding("runtime-health.unreachable", "critical", (error as Error).message, "runtime"),
      ],
    };
  } finally {
...
async function tenantReadinessGate(context: GateContext): Promise<GateResult> {
      ...
      blocked_by: [
        {
          type: "credential",
          name: "DATABASE_URL",
          reason: "Production database URL is required for read-only readiness",
        },
      ],
      assertions: [
        assertion(
          "tenant.database-url",
          "Production DATABASE_URL is provided",
          true,
          false,
          "UNKNOWN",
        ),
  ...
  try {
    ...
    return {
      status: "FAIL",
      assertions: [
        assertion("tenant.database-url-format", "DATABASE_URL is a valid URL", true, false, "FAIL"),
      ],
      findings: [
        finding("tenant.database-url-invalid", "critical", (error as Error).message, "database"),
      ],
    };
  }
  ...
  try {
    ...
    for (const client of result.rows) {
      ...
      const hasToken =
        typeof deployment.githubToken === "string" && deployment.githubToken.trim().length > 0;
      const hasRepo =
        typeof deployment.websiteBotRepo === "string" &&
        deployment.websiteBotRepo.trim().length > 0;
    ...
    const assertions = [
      ...
      assertion(
        "tenant.deploy-config",
        "Deployment credentials are complete pairs",
        [],
        incomplete,
        incomplete.length === 0 ? "PASS" : "FAIL",
      ),
      assertion(
        "tenant.live-mutation-gate",
        "Autonomous live mutation remains disabled",
        true,
        disabled,
        disabled ? "PASS" : "FAIL",
      ),
    ...
    return {
      status: "FAIL",
      assertions: [
        assertion("tenant.query", "Read-only tenant query succeeds", true, false, "FAIL"),
      ],
  ...
  if (!apiUrl || !apiKey)
    return {
      ...
      blocked_by: [
        {
          type: "credential",
          name: "POSTHOG_API_URL/POSTHOG_PERSONAL_API_KEY",
          reason: "Read-only PostHog access is required",
        },
      ],
      assertions: [
        assertion(
          "posthog.credentials",
          "PostHog URL and API key are provided",
          true,
          false,
          "UNKNOWN",
        ),
  ...
  try {
    ...
    const assertions = [
      assertion(
        ...
        "PostHog project list is readable",
        200,
        response.status,
...
export const GATES: GateDefinition[] = [
  ...
  { id: "source", gateClass: "static", dependencies: ["preflight"], execute: sourceGate },
  {
    ...
    dependencies: ["source"],
    execute: (context) =>
      commandGate(context, "typecheck", "npm", ["run", "typecheck"], 10 * 60 * 1000),
  },
  {
    id: "lint",
    gateClass: "build",
    dependencies: ["source"],
    execute: (context) => commandGate(context, "lint", "npm", ["run", "lint"], 10 * 60 * 1000),
  },
  {
    id: "test",
    gateClass: "test",
    dependencies: ["source"],
    execute: (context) =>
      commandGate(context, "test", "npm", ["test"], 15 * 60 * 1000, { NODE_ENV: "test" }),
  },
  ...
  { id: "claims", gateClass: "static", dependencies: ["source"], execute: claimsGate },
  {
    ...
    execute: containerGate,
  },
  { id: "runtime-health", gateClass: "operational", dependencies: [], execute: runtimeHealthGate },
  {
    id: "tenant-readiness",
    gateClass: "operational",
    dependencies: [],
    execute: tenantReadinessGate,
  },
  {
    id: "posthog-readiness",
    gateClass: "integration",
    dependencies: [],
    execute: () => posthogReadinessGate(),
  },
];
