...
import { closeDb, getDb, schema } from "../src/core/database/index.js";
...
async function main() {
  ...
  if (siteDeployDryRun) {
    process.stderr.write(
      "  ⚠️  site_deployment incomplete — this client operates in DRY-RUN (no live writes).\n",
    );
  }
  ...
  process.stdout.write(
    `   Site deployment: ${siteDeployDryRun ? "DRY-RUN (incomplete)" : sdWebsiteBotRepo}\n`,
  );
